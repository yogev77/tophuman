...
**Mobile App — Capacitor WebView Wrapper**

Added `mobile/` directory with Capacitor wrapping `https://www.podiumarena.com` in a native WebView. Android platform fully set up. iOS partially set up (needs one command).

**Files created:**
- `mobile/package.json` — Capacitor deps (core, android, ios, splash-screen, status-bar)
- `mobile/capacitor.config.ts` — App config pointing to live site, dark splash screen
- `mobile/www/index.html` — Minimal fallback (Capacitor requirement)
- `mobile/.gitignore` — Ignores node_modules, build artifacts, Pods
- `mobile/android/` — Full Android project (generated by Capacitor)

**Config:** `appId: com.podiumarena.app`, `appName: Podium Arena`, HTTPS-only, dark splash (#0f172a)
...
```
For release: `./gradlew assembleRelease` (needs signing config)

**iOS setup (TODO — run when ready):**
```bash
cd ~/tophuman/mobile
npx cap add ios
npx cap open ios
```
Then in Xcode:
1. Select `App` target → Signing & Capabilities → pick Apple Developer team
2. Bundle ID should be `com.podiumarena.app`
3. Build & run on simulator: Cmd+R
4. TestFlight: Product → Archive → Distribute App → App Store Connect
